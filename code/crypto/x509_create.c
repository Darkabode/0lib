...
static const char *x509_at_oid_from_name(const char *name, size_t name_len)
{
    ...
    for (cur = x509_attrs; cur->name != NULL; ++cur) {
        if (cur->name_len == name_len && fn_lstrcmpiA(cur->name, name) == 0) {
            break;
        }
    }
    return cur->oid;
...
int x509_string_to_names(asn1_named_data **head, const char *name)
{
    ...
    while (c <= end) {
        ...
        if (!in_tag && (*c == ',' || c == end)) {
            if (asn1_store_named_data(head, oid, strlen(oid), (uint8_t *)s, c - s) == NULL) {
...
int x509_set_extension( asn1_named_data **head, const char *oid, size_t oid_len,
                        int critical, const uint8_t *val, size_t val_len )
{
    asn1_named_data *cur;
    ...
    __movsb( cur->val.p + 1, val, val_len );
 ...
 */
static int x509_write_name( uint8_t **p, uint8_t *start,
...
int x509_write_names( uint8_t **p, uint8_t *start,
...
int x509_write_sig( uint8_t **p, uint8_t *start,
                    const char *oid, size_t oid_len,
...
static int x509_write_extension( uint8_t **p, uint8_t *start,
                                 asn1_named_data *ext )
{
    int ret;
    size_t len = 0;
 ...
 *     }
 */
int x509_write_extensions( uint8_t **p, uint8_t *start,
                           asn1_named_data *first )
{
    int ret;
    size_t len = 0;
