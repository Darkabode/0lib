                    ...
                    size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        __movsb( (void *) (ctx->buffer + left), input, fill );
    ...
    if( ilen > 0 )
        __movsb( (void *) (ctx->buffer + left), input, ilen );
}
 ...
 */
void sha256( const uint8_t *input, size_t ilen,
             uint8_t output[32], int is224 )
{
    ...
    __stosb( &ctx, 0, sizeof( sha256_context ) );
}
...
void sha256_hmac_starts( sha256_context *ctx, const uint8_t *key, size_t keylen, int is224 )
{
    ...
    __stosb( ctx->ipad, 0x36, 64 );
    __stosb( ctx->opad, 0x5C, 64 );
...
void sha256_hmac_finish( sha256_context *ctx, uint8_t output[32] )
{
    ...
    __stosb( tmpbuf, 0, sizeof( tmpbuf ) );
}
 ...
 */
void sha256_hmac( const uint8_t *key, size_t keylen, const uint8_t *input, size_t ilen, uint8_t output[32], int is224 )
{
    sha256_context ctx;
