 ...
 */
int x509write_csr_set_key_usage( x509write_csr *ctx, uint8_t key_usage );
 ...
 *
 * \param ctx       CSR context to memory_free
 */
 ...
 */
int x509write_csr_der( x509write_csr *ctx, uint8_t *buf, size_t size,
 ...
 *                  ECDSA signatures always require a non-NULL f_rng.
 */
int x509write_csr_pem( x509write_csr *ctx, uint8_t *buf, size_t size,
                       int (*f_rng)(void *, uint8_t *, size_t),
                       void *p_rng );
