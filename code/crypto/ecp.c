...
const ecp_curve_info *ecp_curve_info_from_name( const char *name )
{
    ...
    for( curve_info = ecp_curve_list();
         ...
         curve_info++ )
    {
        if( _stricmp( curve_info->name, name ) == 0 )
            return( curve_info );
    }
...
void ecp_group_init( ecp_group *grp )
{
    ...
    __stosb( grp, 0, sizeof( ecp_group ) );
}
 ...
 */
int ecp_point_read_binary( const ecp_group *grp, ecp_point *pt, const uint8_t *buf, size_t ilen )
{
    int ret;
    size_t plen;
 ...
 */
int ecp_tls_read_group( ecp_group *grp, const uint8_t **buf, size_t len )
{
    uint16_t tls_id;
    const ecp_curve_info *curve_info;
 ...
 */
static int ecp_modp( mpi_t *N, const ecp_group *grp )
{
    int ret;
...
/*
 * Reduce a mpi_t mod p in-place, general case, to use after mpi_mul_mpi
 */

#define MOD_MUL( N )    do { MPI_CHK( ecp_modp( &N, grp ) ); } \
                        while( 0 )

/*
 * Reduce a mpi_t mod p in-place, to use after mpi_sub_mpi
...
/*
 * Reduce a mpi_t mod p in-place, to use after mpi_add_mpi and mpi_mul_int.
                                   ...
                                   ecp_point *T[], size_t t_len )
{
    ...
    if( ( c = (mpi_t *) memory_alloc( t_len * sizeof( mpi_t ) ) ) == NULL )
 ...
 */
static void ecp_comb_fixed( uint8_t x[], size_t d,
                            uint8_t w, const mpi_t *m )
{
    size_t i, j;
    uint8_t c, cc, adjust;
...
static int ecp_select_comb( const ecp_group *grp, ecp_point *R,
                            const ecp_point T[], uint8_t t_len,
                            uint8_t i )
{
    int ret;
    uint8_t ii, j;
...
static int ecp_mul_comb( ecp_group *grp, ecp_point *R,
                         const mpi_t *m, const ecp_point *P,
                         int (*f_rng)(void *, uint8_t *, size_t),
                         void *p_rng )
{
    int ret;
    uint8_t w, m_is_odd, p_eq_g, pre_len, i;
    size_t d;
    uint8_t k[COMB_MAX_D + 1];
    ecp_point *T;
    mpi_t M, mm;
    ...
    if( T == NULL )
    {
        T = (ecp_point *) memory_alloc( pre_len * sizeof( ecp_point ) );
        if( T == NULL )
        {
...
/*
 * Check that an mpi_t is valid as a private key
 */
int ecp_check_privkey( const ecp_group *grp, const mpi_t *d )
{
#if defined(POLARSSL_ECP_MONTGOMERY)
    if( ecp_get_type( grp ) == POLARSSL_ECP_TYPE_MONTGOMERY )
    {
 ...
 * Generate a keypair
 */
int ecp_gen_keypair( ecp_group *grp, mpi_t *d, ecp_point *Q,
                     int (*f_rng)(void *, uint8_t *, size_t),
                     void *p_rng )
{
    int ret;
    ...
    if( ecp_get_type( grp ) == POLARSSL_ECP_TYPE_SHORT_WEIERSTRASS )
    {
        ...
        int count = 0;
        uint8_t rnd[POLARSSL_ECP_MAX_BYTES];
