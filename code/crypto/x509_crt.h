...
typedef struct _x509_crt
{
    ...
    uint8_t key_usage;    /**< Optional key usage extension value: See the values in x509.h */

    x509_sequence ext_key_usage; /**< Optional list of extended key usage OIDs. */

    uint8_t ns_cert_type; /**< Optional Netscape certificate type extension value: See the values in x509.h */
 ...
 *
 * \param crt      Certificate chain to memory_free
 */
 ...
 *
 * \param ctx       CRT context to memory_free
 */
 ...
 *                  ECDSA signatures always require a non-NULL f_rng.
 */
int x509write_crt_pem( x509write_cert *ctx, uint8_t *buf, size_t size,
                       int (*f_rng)(void *, uint8_t *, size_t),
                       void *p_rng );
