 ...
 * NONE must be zero so that __stosb()ing structure to zero works */
...
typedef int (*rsa_sign_func)( void *ctx,
                     ...
                     int mode, md_type_t md_alg, uint32_t hashlen,
...
struct _ssl_session
{
    ...
    uint8_t mfl_code;     /*!< MaxFragmentLength negotiated by peer */
...
struct _ssl_transform
{
    ...
    uint32_t keylen;                /*!<  symmetric key length    */
...
struct _ssl_handshake_params
{
    ...
    void (*update_checksum)(ssl_context *, const uint8_t *, size_t);
    void (*calc_verify)(ssl_context *, uint8_t *);
    void (*calc_finished)(ssl_context *, uint8_t *, int);
    int  (*tls_prf)(const uint8_t *, size_t, const char *,
    ...
    uint8_t premaster[POLARSSL_PREMASTER_SIZE];
                                        /*!<  premaster secret        */
...
struct _ssl_context
{
...
#if defined(POLARSSL_SSL_SERVER_NAME_INDICATION)
    int (*f_sni)(void *, ssl_context *, const uint8_t *, size_t);
    ...
    uint8_t *in_iv;       /*!< ivlen-byte IV (in_hdr+5)         */
    uint8_t *in_msg;      /*!< message contents (in_iv+ivlen)   */
    ...
    uint8_t *out_iv;      /*!< ivlen-byte IV (out_hdr+5)        */
    uint8_t *out_msg;     /*!< message contents (out_iv+ivlen)  */
...
#if defined(POLARSSL_SSL_MAX_FRAGMENT_LENGTH)
    uint8_t mfl_code;     /*!< MaxFragmentLength chosen by us   */
#endif /* POLARSSL_SSL_MAX_FRAGMENT_LENGTH */
 ...
 *                 that the rsa_context_t * is a void * in the callbacks)
 *
 *                 Note: own_cert should contain IN order from the bottom
 *                 up your certificate chain. The top certificate (self-signed)
 *                 can be omitted.
 ...
 *                 const uint8_t *hostname, size_t len). If a suitable
...
void ssl_set_sni( ssl_context *ssl,
                  int (*f_sni)(void *, ssl_context *, const uint8_t *,
                               size_t),
                  void *p_sni );
