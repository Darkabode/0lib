 ...
 *  }
 */
static int pk_write_rsa_pubkey( uint8_t **p, uint8_t *start,
                                  rsa_context_t *rsa )
{
    int ret;
    size_t len = 0;
 ...
 */
int pk_write_ec_pubkey( uint8_t **p, uint8_t *start, ecp_keypair *ec )
{
    int ret;
    size_t len = 0;
    uint8_t buf[POLARSSL_ECP_MAX_PT_LEN];
 ...
 * }
 */
int pk_write_ec_param( uint8_t **p, uint8_t *start, ecp_keypair *ec )
{
    int ret;
    size_t len = 0;
    const char *oid;
...
int pk_write_pubkey( uint8_t **p, uint8_t *start, const pk_context *key )
{
    int ret;
    size_t len = 0;
    ...
    else if( pk_get_type( key ) == POLARSSL_PK_ECKEY )
...
int pk_write_pubkey_der( pk_context *key, uint8_t *buf, size_t size )
{
    int ret;
    uint8_t *c;
...
int pk_write_key_der( pk_context *key, uint8_t *buf, size_t size )
{
    int ret;
    uint8_t *c = buf + size;
    size_t len = 0;
...
int pk_write_pubkey_pem( pk_context *key, uint8_t *buf, size_t size )
{
    int ret;
...
int pk_write_key_pem( pk_context *key, uint8_t *buf, size_t size )
{
    int ret;
    uint8_t output_buf[4096];
