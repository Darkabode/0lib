 ...
 * build lists of t_uint's from lists of uint8_t's grouped by 8, 4 or 2
 */
 ...
 */
static void ecp_mpi_load( mpi_t *X, const t_uint *p, size_t len )
{
    X->s = 1;
 ...
 */
static void ecp_mpi_set1( mpi_t *X )
{
    static t_uint one[] = { 1 };
    X->s = 1;
    X->n = 1;
...
#if defined(POLARSSL_ECP_DP_SECP192R1_ENABLED)
static int ecp_mod_p192( mpi_t * );
#endif
#if defined(POLARSSL_ECP_DP_SECP224R1_ENABLED)
static int ecp_mod_p224( mpi_t * );
#endif
#if defined(POLARSSL_ECP_DP_SECP256R1_ENABLED)
static int ecp_mod_p256( mpi_t * );
#endif
#if defined(POLARSSL_ECP_DP_SECP384R1_ENABLED)
static int ecp_mod_p384( mpi_t * );
#endif
#if defined(POLARSSL_ECP_DP_SECP521R1_ENABLED)
static int ecp_mod_p521( mpi_t * );
#endif
...
static int ecp_mod_p255( mpi_t * );
#endif
#if defined(POLARSSL_ECP_DP_SECP192K1_ENABLED)
static int ecp_mod_p192k1( mpi_t * );
#endif
#if defined(POLARSSL_ECP_DP_SECP224K1_ENABLED)
static int ecp_mod_p224k1( mpi_t * );
#endif
#if defined(POLARSSL_ECP_DP_SECP256K1_ENABLED)
static int ecp_mod_p256k1( mpi_t * );
#endif
 ...
 */
static int ecp_mod_p192( mpi_t *N )
{
    int ret;
    t_uint c = 0;
    t_uint *p, *end;
...
#if defined(POLARSSL_HAVE_INT32)  /* 32 bit */
...
#define INIT( b )                                           \
    ...
    C.p = Cp;                                               \
    __stosb( Cp, 0, C.n * sizeof( t_uint ) );                \
                                                            \
 ...
 */
static int fix_negative( mpi_t *N, signed char c, mpi_t *C, size_t bits )
{
    int ret;
 ...
 */
static int ecp_mod_p224( mpi_t *N )
{
    INIT( 224 );
 ...
 */
static int ecp_mod_p256( mpi_t *N )
{
    INIT( 256 );
 ...
 */
static int ecp_mod_p384( mpi_t *N )
{
    INIT( 384 );
 ...
 */
static int ecp_mod_p521( mpi_t *N )
{
    int ret;
    size_t i;
    mpi_t M;
    ...
    if( M.n > P521_WIDTH + 1 )
        M.n = P521_WIDTH + 1;
    M.p = Mp;
    __movsb( Mp, N->p + P521_WIDTH - 1, M.n * sizeof( t_uint ) );
 ...
 */
static int ecp_mod_p255( mpi_t *N )
{
    int ret;
    size_t i;
    mpi_t M;
    ...
    if( M.n > P255_WIDTH + 1 )
        M.n = P255_WIDTH + 1;
    M.p = Mp;
    __stosb( Mp, 0, sizeof Mp );
    __movsb( Mp, N->p + P255_WIDTH - 1, M.n * sizeof( t_uint ) );
...
static int ecp_mod_koblitz( mpi_t *N, t_uint *Rp, size_t p_limbs,
                                   size_t adjust, size_t shift, t_uint mask )
{
    ...
    if( M.n > p_limbs + adjust )
        M.n = p_limbs + adjust;
    __stosb( Mp, 0, sizeof Mp );
    __movsb( Mp, N->p + p_limbs - adjust, M.n * sizeof( t_uint ) );
    if (shift != 0 )
 ...
 */
static int ecp_mod_p192k1( mpi_t *N )
{
    static t_uint Rp[] = {
 ...
 */
static int ecp_mod_p224k1( mpi_t *N )
{
    static t_uint Rp[] = {
 ...
 */
static int ecp_mod_p256k1( mpi_t *N )
{
    static t_uint Rp[] = {
