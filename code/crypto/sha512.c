...
void sha512( const uint8_t *input, size_t ilen, uint8_t output[64], int is384 )
{
    ...
    __stosb( &ctx, 0, sizeof( sha512_context ) );
}
...
void sha512_hmac_starts( sha512_context *ctx, const uint8_t *key, size_t keylen, int is384 )
{
    ...
    __stosb( ctx->ipad, 0x36, 128 );
    __stosb( ctx->opad, 0x5C, 128 );
 ...
 */
void sha512_hmac(const uint8_t *key, size_t keylen, const uint8_t *input, size_t ilen, uint8_t output[64], int is384)
{
    sha512_context ctx;
