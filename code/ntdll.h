...
#define INIT_UNICODE_STRING(us, wch)                 \
    ...
    us.Length        = (USHORT)(fn_lstrlenW(wch) * sizeof(wchar_t)); \
    us.Buffer        = wch
#endif
...
{
   struct _RTL_HANDLE_TABLE_ENTRY *Next;    /* pointer to next memory_free handle */
   PVOID  Object;
...
typedef struct _PEB
{
    ...
    // to point to the first memory_free byte after the PEB and MaximumNumberOfHeaps
