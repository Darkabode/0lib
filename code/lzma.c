...
static int LzInWindow_Create(CMatchFinder *p, uint32_t keepSizeReserv)
{
    ...
    if (p->bufferBase == 0 || p->blockSize != blockSize)
    {
        LzInWindow_Free(p);
        p->blockSize = blockSize;
        p->bufferBase = (uint8_t*)memory_alloc((size_t)blockSize);
    }
...
{
    fn_RtlMoveMemory(p->bufferBase, p->buffer - p->keepSizeBefore, (size_t)(p->streamPos - p->pos + p->keepSizeBefore));
...
static CLzRef* AllocRefs(uint32_t num)
{
    ...
    return (CLzRef*)memory_alloc(sizeInBytes);
}

int MatchFinder_Create(CMatchFinder *p, uint32_t historySize, uint32_t keepAddBufferBefore, uint32_t matchMaxLen, uint32_t keepAddBufferAfter)
{
    uint32_t sizeReserv;
...
static int RangeEnc_Alloc(CRangeEnc *p)
{
  if (p->bufBase == 0)
  {
    p->bufBase = (uint8_t*)memory_alloc(RC_BUF_SIZE);
    if (p->bufBase == 0)
      return 0;
...
{
  memory_free(p->litProbs);
  memory_free(p->saveState.litProbs);
  p->litProbs = 0;
...
static int LzmaEnc_Alloc(CLzmaEnc *p, uint32_t keepWindowSize)
{
...

  {
    ...
    {
      LzmaEnc_FreeLits(p);
      p->litProbs = (uint32_t *)memory_alloc((0x300 << lclp) * sizeof(uint32_t));
      p->saveState.litProbs = (uint32_t *)memory_alloc((0x300 << lclp) * sizeof(uint32_t));
...
static size_t MyWrite(void *pp, const void *data, size_t size)
{
  ...
  __movsb(p->data, (const uint8_t*)data, size);
  p->rem -= size;
  p->data += size;
  return size;
}
...
void lzma_encprops_init(CLzmaEncProps *p)
{
    //  p->mc = 0;
...
int lzma_encode(uint8_t* dest, size_t* destLen, const uint8_t* src, size_t srcLen, const CLzmaEncProps* props, uint8_t* propsEncoded, size_t* propsSize)
{
    int res;
    CLzmaEnc *p = (CLzmaEnc*)memory_alloc(sizeof(CLzmaEnc));

    p->rc.outStream = 0;
    p->rc.bufBase = 0;

    uint32_t i;
    p->matchFinderBase.bufferBase = 0;
    p->matchFinderBase.directInput = 0;
    p->matchFinderBase.hash = 0;
    p->matchFinderBase.bigHash = 0;
    ...
    for (i = (1 << kNumMoveReducingBits) / 2; i < kBitModelTotal; i += (1 << kNumMoveReducingBits)) {
        ...
        p->ProbPrices[i >> kNumMoveReducingBits] = ((kNumBitModelTotalBits << kCyclesBits) - 15 - bitCount);
    }
    ...
    if (res == SZ_OK) {
        ...
        if (*propsSize < LZMA_PROPS_SIZE) {
            ...
            *propsSize = LZMA_PROPS_SIZE;
            propsEncoded[0] = (uint8_t)((p->pb * 5 + p->lp) * 9 + p->lc);
            ...
            for (i = 0; i < 4; ++i) {
                propsEncoded[1 + i] = (uint8_t)(dictSize >> (8 * i));
            }
            res = SZ_OK;
        }
        if (res == SZ_OK) {
...
int lzma_decode(uint8_t* outBuffer, uint32_t* pOutSize, const uint8_t* inBuffer, uint32_t inSize, ELzmaStatus* status)
{
    ...
    *pOutSize = 0;
    ...
    probs = (uint32_t*)memory_alloc(((uint32_t)LZMA_BASE_SIZE + (LZMA_LIT_SIZE << (lc + lp))) << 2);
    ...
    *status = LZMA_STATUS_NOT_SPECIFIED;
    ...
    __stosd((unsigned long*)probs, kBitModelTotal >> 1, Literal + ((uint32_t)LZMA_LIT_SIZE << (lc + lp)));
...
int lzma_auto_decode(uint8_t* inStream, uint32_t inSize, uint8_t** outStream, uint32_t* poutSize)
{
    ...
    do {
        ...
        do {
            if (*outStream != NULL) {
                memory_free(*outStream);
            }
            outSize *= 2;
            origOutSize = outSize;
            *outStream = (uint8_t*)memory_alloc(outSize);
            ret = lzma_decode(*outStream, &outSize, inStream, inSize, &st);
        } while (ret == SZ_OK && outSize == origOutSize);
