...
void zfs_put_boot(zfsparams * ft, uint8_t* boot)
{
    int cnt = 0;
    ...
    boot[cnt++] = 0x3c;
    boot[cnt++] = 0x90;
    __stosb(boot + cnt, 0x20, 8); /* system id */
    cnt += 8;
    ...
    *(__int32 *)(boot + cnt) = ft->zfs_length; cnt += 4;	/* zfs size 32 */
    ...
    boot[cnt++] = 0x00;
    __stosb(boot+cnt, 0, 12); cnt+=12;	/* Reserved */
    ...
    *(uint32_t*)(boot + cnt) = 0xAABBCCDD;		/* vol id */
    ...
    __stosb(boot + cnt, 0x20, 11);	/* vol title */
    cnt += 11;
...
//     *(uint32_t*)(boot + cnt + sizeof(uint32_t)) = 0x31313538;
    ...
    __stosb (boot + cnt, 0, 420);	/* boot code */
    cnt += 420;
    boot[cnt++] = 0x55;
    boot[cnt++] = 0xaa;	/* boot sig */
}

void zfs_put_fs_info(uint8_t* sector, zfsparams *ft)
{
    

    __stosb(sector, 0, BDEV_BLOCK_SIZE);
...
int zfs_format(pzfs_io_manager_t pIoman)
{
	...
	__stosb(sector, 0, 512);

	zfs_put_boot(&ft, (uint8_t*)sector);

	if (bdev_write((const uint8_t*)sector, nSecNo++, 1, pIoman->pbs) != ERR_OK) {
		goto fail;
	}

	/* zfs32 boot area */
	/* fsinfo */
	zfs_put_fs_info((uint8_t* ) sector, &ft);
	if (bdev_write(sector, nSecNo++, 1, pIoman->pbs) != ERR_OK) {
		goto fail;
	}
	...
	/* reserved */
	while (nSecNo < (uint32_t)ft.reserved) {
		__stosb(sector, 0, BDEV_BLOCK_SIZE);
		if (bdev_write(sector, nSecNo++, 1, pIoman->pbs) != ERR_OK) {
			goto fail;
		}
