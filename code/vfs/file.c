...
int zfs_open(pzfs_io_manager_t pIoman, pzfs_file_t* ppFile, const char* path, uint8_t mode, uint8_t special)
{
    ...
    *ppFile = NULL;
    ...
    pFile = memory_alloc(sizeof(zfs_file_t));

    do {
        __stosb(pFile, 0, sizeof(zfs_file_t));
        // Get the Mode Bits.
        pFile->mode = mode;
		...
		fn_lstrcpynA(filename, (path + i + 1), ZFS_MAX_FILENAME + 1);
        ...
        if (dirCluster > 0) {
            fileCluster = zfs_find_entry_in_dir(pIoman, dirCluster, filename, 0x00, &dirEntry, &err);
            if (ZFS_isERR(err)) {
                break;
            }

            if (fileCluster == 0) {    // If 0 was returned, it might be because the file has no allocated cluster
				if (fn_lstrlenA(filename) == fn_lstrlenA(dirEntry.fileName)) {
                    if (dirEntry.filesize == 0 && zfs_strmatch(filename, dirEntry.fileName, (uint16_t)fn_lstrlenA(filename)) == TRUE) {
            ...
            if (fileCluster == 0) {
                if ((mode & ZFS_MODE_CREATE)) {
                    fileCluster = zfs_create_file(pIoman, dirCluster, filename, &dirEntry, special, &err);
                    if (ZFS_isERR(err)) {
                        break;
                    }
...
char zfs_is_dir_empty(pzfs_io_manager_t pIoman, const char* path, uint8_t special)
{
    ...
    err = zfs_findfirst(pIoman, &MyDir, path, special);
    while (err == 0) {
        i++;
        err = zfs_findnext(pIoman, &MyDir, special);
        if (i > 2) {
            return FALSE;
        }
    }
...
int zfs_rmdir(pzfs_io_manager_t pIoman, const char* path, uint8_t special)
{
    ...
    err = zfs_open(pIoman, &pFile, path, ZFS_MODE_DIR, special);
    ...
    zfs_lock_dir(pIoman);
    if (zfs_is_dir_empty(pIoman, path, special)) {
        zfs_lock(pIoman);
        err = zfs_unlink_cluster_chain(pIoman, pFile->objectCluster);
        ...
        err = zfs_init_entry_fetch(pIoman, pFile->dirCluster, &FetchContext);
        if (ZFS_isERR(err)) {
            zfs_unlock_dir(pIoman);
            zfs_close(pFile);
            return err;
        ...
        err = zfs_rm_lfns(pIoman, pFile->dirEntry, &FetchContext);
        if (ZFS_isERR(err)) {
        ...
        err = zfs_fetch_entry_with_context(pIoman, pFile->dirEntry, &FetchContext, EntryBuffer);
        if (ZFS_isERR(err)) {
            zfs_cleanup_entry_fetch(pIoman, &FetchContext);
            zfs_unlock_dir(pIoman);
            zfs_close(pFile);
        ...
        EntryBuffer[0] = 0xE5;
        err = zfs_push_entry_with_context(pIoman, pFile->dirEntry, &FetchContext, EntryBuffer);
        if (ZFS_isERR(err)) {
            zfs_cleanup_entry_fetch(pIoman, &FetchContext);
            zfs_unlock_dir(pIoman);
            zfs_close(pFile);
        ...
        err = zfs_increase_free_clusters(pIoman, pFile->iChainLength);
        if (ZFS_isERR(err)) {
            zfs_cleanup_entry_fetch(pIoman, &FetchContext);
            zfs_unlock_dir(pIoman);
            zfs_close(pFile);
    ...
    zfs_unlock_dir(pIoman);
    err = zfs_close(pFile); // Free the file pointer resources
    if (ZFS_isERR(err)) {
        return err;
    }
...
int zfs_unlink(pzfs_io_manager_t pIoman, const char* path, uint8_t special)
{
    ...
    err = zfs_open(pIoman, &pFile, path, ZFS_MODE_READ, special);
    ...
    if (pFile->objectCluster) {    // Ensure there is actually a cluster chain to delete!
        zfs_lock(pIoman);    // Lock the ZFS so its thread-safe.
        err = zfs_unlink_cluster_chain(pIoman, pFile->objectCluster);
...
int zfs_move(pzfs_io_manager_t pIoman, const char* szSrcFile, const char* szDstFile, uint8_t special)
{
    ...
    err = zfs_open(pIoman, &pDestFile, szDstFile, ZFS_MODE_READ, special);
    ...
    err = zfs_open(pIoman, &pSrcFile, szSrcFile, ZFS_MODE_READ, special);

    if (ZFS_GETERROR(err) == ZFS_ERR_FILE_OBJECT_IS_A_DIR) {
        // Open a directory for moving!
        err = zfs_open(pIoman, &pSrcFile, szSrcFile, ZFS_MODE_DIR, special);
    }
    ...
    err = zfs_init_entry_fetch(pIoman, pSrcFile->dirCluster, &fetchContext);
    if (ZFS_isERR(err)) {
        zfs_close(pSrcFile);
        return err;
    }
    err = zfs_fetch_entry_with_context(pIoman, pSrcFile->dirEntry, &fetchContext, entryBuffer);
    if (ZFS_isERR(err)) {
        zfs_close(pSrcFile);
        zfs_cleanup_entry_fetch(pIoman, &fetchContext);
        return err;
    ...
    i = (uint16_t)fn_lstrlenA(szDstFile);
	...
	fn_lstrcpynA(fileEntry.fileName, (szDstFile + i + 1), ZFS_MAX_FILENAME + 1);

    if (i == 0) {
        i = 1;
    }

    dirCluster = zfs_find_dir(pIoman, szDstFile, i, special, &err);
    if (ZFS_isERR(err)) {
        zfs_close(pSrcFile);
        zfs_cleanup_entry_fetch(pIoman, &fetchContext);
        return err;
    ...
    if (dirCluster) {
        ...
        // Edit the Directory Entry! (So it appears as deleted);
        zfs_lock_dir(pIoman);
        err = zfs_rm_lfns(pIoman, pSrcFile->dirEntry, &fetchContext);
        if (ZFS_isERR(err)) {
            zfs_unlock_dir(pIoman);
            zfs_close(pSrcFile);
            zfs_cleanup_entry_fetch(pIoman, &fetchContext);
        ...
        err = zfs_push_entry_with_context(pIoman, pSrcFile->dirEntry, &fetchContext, entryBuffer);
        if (ZFS_isERR(err)) {
            zfs_unlock_dir(pIoman);
            zfs_close(pSrcFile);
            zfs_cleanup_entry_fetch(pIoman, &fetchContext);
...
uint32_t zfs_get_sequential_clusters(pzfs_io_manager_t pIoman, uint32_t StartCluster, uint32_t Limit, int*pError)
{
    ...
    *pError = ERR_OK;
    ...
    do {
        ...
        NextCluster = zfs_get_entry(pIoman, CurrentCluster, pError);
        if (*pError) {
            return 0;
        }
...
int zfs_read_clusters(pzfs_file_t pFile, uint32_t Count, uint8_t *buffer)
{
    ...
    while (Count != 0) {
        if ((Count - 1) > 0) {
            SequentialClusters = zfs_get_sequential_clusters(pFile->pIoman, pFile->addrCurrentCluster, (Count - 1), &Error);
            if (ZFS_isERR(Error)) {
                return Error;
            }
        }
        ulSectors = (SequentialClusters + 1) * ZFS_SECTORS_PER_CLUSTER;
        nItemLBA = zfs_cluster_to_lba(pFile->pIoman, pFile->addrCurrentCluster);

        slRetVal = zfs_read_block(pFile->pIoman, nItemLBA, ulSectors, buffer);
        if (slRetVal < 0) {
            return slRetVal;
        }
        
        Count -= (SequentialClusters + 1);
        pFile->addrCurrentCluster = zfs_traverse(pFile->pIoman, pFile->addrCurrentCluster, (SequentialClusters + 1), &Error);
        if (ZFS_isERR(Error)) {
            return Error;
        }
...
int zfs_extend_file(pzfs_file_t pFile, uint32_t Size)
{
    ...
    if (pFile->filesize == 0 && pFile->objectCluster == 0) {    // No Allocated clusters.
        ...
        pFile->addrCurrentCluster = zfs_create_cluster_chain(pFile->pIoman, &Error);

        if (ZFS_isERR(Error)) {
            return Error;
        }

        Error = zfs_get_dir_entry(pIoman, pFile->dirEntry, pFile->dirCluster, &OriginalEntry);
        
        if (!Error) {
            OriginalEntry.objectCluster = pFile->addrCurrentCluster;
            Error = zfs_put_dir_entry(pIoman, pFile->dirEntry, pFile->dirCluster, &OriginalEntry);
        }
    ...
    if (pFile->iChainLength == 0) {    // First extension requiring the chain length, 
        pFile->iChainLength = zfs_get_chain_length(pIoman, pFile->objectCluster, &pFile->iEndOfChain, &Error);
        if (ZFS_isERR(Error)) {
            return Error;
        }
    }
    ...
    if (nTotalClustersNeeded > pFile->iChainLength) {
        ...
        for (i = 0; i < nClusterToExtend; i++) {
            CurrentCluster = zfs_find_end_of_chain(pIoman, NextCluster, &Error);
            if (ZFS_isERR(Error)) {
                zfs_unlock(pIoman);
                zfs_decrease_free_clusters(pIoman, i);
                return Error;
            }
        ...
        pFile->iEndOfChain = zfs_find_end_of_chain(pIoman, NextCluster, &Error);
        if (ZFS_isERR(Error)) {
            zfs_unlock(pIoman);
            zfs_decrease_free_clusters(pIoman, i);
            return Error;
        ...
        Error = zfs_decrease_free_clusters(pIoman, i);    // Keep Tab of Numbers for fast FreeSize()
        if (ZFS_isERR(Error)) {
            return Error;
        }
        Error = zfs_flush_cache(pIoman);
        if (Error) {
            return Error;
        }
    }
...
int zfs_write_clusters(pzfs_file_t pFile, uint32_t Count, uint8_t *buffer)
{
    ...
    while (Count != 0) {
        ...
        slRetVal = zfs_write_block(pFile->pIoman, nItemLBA, ulSectors, buffer);
...
int zfs_read(pzfs_file_t pFile, uint8_t* buffer, uint32_t size, uint32_t* pReaded)
{
    ...
    *pReaded = 0;
    ...
    nClusterDiff = zfs_get_cluster_chain_number(pFile->filePointer, 1) - pFile->currentCluster;
    if (nClusterDiff) {
        if (pFile->currentCluster < zfs_get_cluster_chain_number(pFile->filePointer, 1)) {
            pFile->addrCurrentCluster = zfs_traverse(pIoman, pFile->addrCurrentCluster, nClusterDiff, &err);
            if (ZFS_isERR(err)) {
                return err;
            }
    ...
    nRelBlockPos = zfs_get_minor_block_entry(pFile->filePointer, 1); // Get the position within a block.
    
    nItemLBA = zfs_cluster_to_lba(pIoman, pFile->addrCurrentCluster);
    nItemLBA = nItemLBA + zfs_get_major_block_number(pFile->filePointer, 1) + zfs_get_minor_block_number(pFile->filePointer, 1);

    if ((nRelBlockPos + size) < BDEV_BLOCK_SIZE) {    // Bytes to read are within a block and less than a block size.
        pBuffer = zfs_get_buffer(pIoman, nItemLBA, ZFS_MODE_READ);
        if (!pBuffer) {
            return (ZFS_ERR_DEVICE_DRIVER_FAILED | ZFS_READ);
        }
        __movsb(buffer, (pBuffer->pBuffer + nRelBlockPos), size);
        ...
        *pReaded = size;
        ...
        if (nRelBlockPos != 0) {    // Not on a sector boundary, at this point the LBA is known.
            ...
            __movsb(buffer, (pBuffer->pBuffer + nRelBlockPos), nBytesToRead);
            zfs_release_buffer(pIoman, pBuffer);
        ...
        nRelClusterPos = zfs_get_cluster_position(pFile->filePointer, 1);
        ...
        if (nRelClusterPos != 0 && size >= nBytesPerCluster) { // Need to get to cluster boundary
            ...
            ret = zfs_read_block(pIoman, nItemLBA, (uint32_t) sSectors, buffer);
            if (ret < 0) {
                return ret;
            }
        ...
        if (size >= nBytesPerCluster) {
            ...
            ret = zfs_read_clusters(pFile, (size / nBytesPerCluster), buffer);
            if (ret < 0) {
                return ret;
            }
        ...
        if (size > 0) {
            ...
            __movsb(buffer, pBuffer->pBuffer, size);
            zfs_release_buffer(pIoman, pBuffer);
...
int zfs_getc(pzfs_file_t pFile)
{
    ...
    relMinorBlockPos = zfs_get_minor_block_entry(pFile->filePointer, 1);
    clusterNum = zfs_get_cluster_chain_number(pFile->filePointer, 1);

    nClusterDiff = zfs_get_cluster_chain_number(pFile->filePointer, 1) - pFile->currentCluster;
    if (nClusterDiff) {
        if (pFile->currentCluster < zfs_get_cluster_chain_number(pFile->filePointer, 1)) {
            pFile->addrCurrentCluster = zfs_traverse(pFile->pIoman, pFile->addrCurrentCluster, nClusterDiff, &err);
            if (ZFS_isERR(err)) {
                return err;
            }
            pFile->currentCluster += nClusterDiff;
    ...
    fileLBA = zfs_cluster_to_lba(pFile->pIoman, pFile->addrCurrentCluster)    + zfs_get_major_block_number(pFile->filePointer, (uint16_t) 1);
    fileLBA = fileLBA + zfs_get_minor_block_number(pFile->filePointer, 1);

    pBuffer = zfs_get_buffer(pFile->pIoman, fileLBA, ZFS_MODE_READ);
    if (!pBuffer) {
        return (ZFS_ERR_DEVICE_DRIVER_FAILED | ZFS_GETC);
    }
    retChar = pBuffer->pBuffer[relMinorBlockPos];
    zfs_release_buffer(pFile->pIoman, pBuffer);
...
int zfs_getline(pzfs_file_t pFile, char* szLine, uint32_t maxLen)
{
    ...
    for (i = 0; i < (maxLen - 1) && (c = zfs_getc(pFile)) >= 0 && c != '\n'; ++i) {
        if (c == '\r') {
            i--;
        }
        else {
...
int zfs_write(pzfs_file_t pFile, uint8_t* buffer, uint32_t size, uint32_t* pWritten)
{
    ...
    err = zfs_extend_file(pFile, pFile->filePointer + size + 1);
    ...
    if ((nRelBlockPos + size) < BDEV_BLOCK_SIZE) {    // Bytes to write are within a block and less than a block size.
        if (!nRelBlockPos && pFile->filePointer >= pFile->filesize) {
            pBuffer = zfs_get_buffer(pIoman, nItemLBA, ZFS_MODE_WR_ONLY);
        }
        else {
            pBuffer = zfs_get_buffer(pIoman, nItemLBA, ZFS_MODE_WRITE);
        }
        if (!pBuffer) {
            return (ZFS_ERR_DEVICE_DRIVER_FAILED | ZFS_WRITE);
        }
        __movsb((pBuffer->pBuffer + nRelBlockPos), buffer, size);
        zfs_release_buffer(pIoman, pBuffer);
        ...
        *pWritten = size;
        ...
        if (nRelBlockPos != 0) {    // Not on a sector boundary, at this point the LBA is known.
            ...
            // Here we copy to the sector boudary.
            __movsb((pBuffer->pBuffer + nRelBlockPos), buffer, nBytesToWrite);
            zfs_release_buffer(pIoman, pBuffer);
        ...
        if (nRelClusterPos != 0 && size >= nBytesPerCluster) { // Need to get to cluster boundary
            ...
            slRetVal = zfs_write_block(pFile->pIoman, nItemLBA, sSectors, buffer);
            if (slRetVal < 0) {
                return slRetVal;
            }
        ...
        if (size >= nBytesPerCluster) {
            ...
            slRetVal = zfs_write_clusters(pFile, nClusters, buffer);
            if (slRetVal < 0) {
                return slRetVal;
            }
        ...
        if (size > 0) {            
            ...
            __movsb(pBuffer->pBuffer, buffer, size);
            zfs_release_buffer(pIoman, pBuffer);
...
int zfs_putc(pzfs_file_t pFile, uint8_t pa_cValue)
{
    ...
    if ((pFile->mode & ZFS_MODE_APPEND)) {
        if (pFile->filePointer < pFile->filesize) {
            err = zfs_seek(pFile, 0, ZFS_SEEK_END);
            if (ZFS_isERR(err)) {
                return err;
            }
        }
    ...
    iRelPos = zfs_get_minor_block_entry(pFile->filePointer, 1);
    
    // Handle File Space Allocation.
    err = zfs_extend_file(pFile, pFile->filePointer + 1);
    if (ZFS_isERR(err)) {
        return err;
    }
    ...
    iItemLBA = zfs_cluster_to_lba(pFile->pIoman, pFile->addrCurrentCluster) + zfs_get_major_block_number(pFile->filePointer, 1);
    iItemLBA = iItemLBA + zfs_get_minor_block_number(pFile->filePointer, 1);
    
    pBuffer = zfs_get_buffer(pFile->pIoman, iItemLBA, ZFS_MODE_WRITE);
    if (!pBuffer) {
...
{    
    ...
    err = zfs_flush_cache(pFile->pIoman);
    if (ZFS_isERR(err)) {
        return err;
    }
    ...
    if (origin == ZFS_SEEK_SET) {
        if (offset >= 0 && (uint32_t)offset <= pFile->filesize) {
            ...
            pFile->currentCluster = zfs_get_cluster_chain_number(pFile->filePointer, 1);
            pFile->addrCurrentCluster = zfs_traverse(pFile->pIoman, pFile->objectCluster, pFile->currentCluster, &err);
            if (ZFS_isERR(err)) {
                return err;
            }
        }
...
int zfs_set_end_of_file(pzfs_file_t pFile)
{
    ...
    while (!zfs_is_end_of_chain(zfsEntry)) {
        zfsEntry = zfs_get_entry(pFile->pIoman, currentCluster, &err);
        if (err) {
            return err;
        }
    ...
    if (num == neededClusters) {
        err = zfs_unlink_cluster_chain(pFile->pIoman, zfsEntry);
        if (err == ERR_OK) {
            err = zfs_put_entry(pFile->pIoman, currentCluster, 0xFFFFFFFF);
        }
    }

    if (err == ERR_OK) {
        pFile->filesize = pFile->filePointer;
        err = zfs_flush_cache(pFile->pIoman); // Ensure all modfied blocks are flushed to disk!
    }
...
int zfs_set_time(pzfs_io_manager_t pIoman, const char* path, uint32_t unixTime, uint32_t aWhat, uint8_t special)
{
    ...
    dirCluster = zfs_find_dir(pIoman, path, (uint16_t) i, special, &err);
    if (err)
        return err;
    ...
    fileCluster = zfs_find_entry_in_dir(pIoman, dirCluster, filename, 0, &origEntry, &err);
    if (err)
        return err;
    ...
    err = zfs_put_dir_entry(pIoman, origEntry.currentItem-1, dirCluster, &origEntry);

    if (err == ERR_OK) {
        err = zfs_flush_cache(pIoman); // Ensure all modfied blocks are flushed to disk!
    }
    return err;
}
...
int zfs_close(pzfs_file_t pFile)
{
    ...
    if (!(pFile->validFlags & ZFS_VALID_FLAG_DELETED) && (pFile->mode & (ZFS_MODE_WRITE |ZFS_MODE_APPEND))) {
        // Update the Dirent!
        err = zfs_get_dir_entry(pFile->pIoman, pFile->dirEntry, pFile->dirCluster, &originalEntry);
        // Error might be non-zero, but don't forget to remove handle from list
        // and to memory_free the pFile pointer
        if (!err && pFile->filesize != originalEntry.filesize) {
            originalEntry.filesize = pFile->filesize;
            err = zfs_put_dir_entry(pFile->pIoman, pFile->dirEntry, pFile->dirCluster, &originalEntry);
        }
    }
    if (!ZFS_GETERROR (err)) {
        err = zfs_flush_cache(pFile->pIoman);        // Ensure all modfied blocks are flushed to disk!
    ...
    // Handle Linked list!
	fn_WaitForSingleObject(pFile->pIoman->mutex, INFINITE);
    pFileChain = (pzfs_file_t ) pFile->pIoman->firstFile;
    if (pFileChain == pFile) {
	...
	fn_ReleaseMutex(pFile->pIoman->mutex);

    // If file written, flush to disk
    memory_free(pFile);
    // Simply memory_free the pointer!
    return err;
}
